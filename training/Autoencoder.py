...
def AE(
    ...
    if (len(dimensions) - 1) != len(activations):
        raise ValueError(
            "There must be as many activation functions as there are layer dimensions minus 1"
        )

    if len(alphas) != (len(dimensions) - 2):
        raise ValueError(
            "There must be as many alpha values as there are layer dimensions minus 2 (here {0})".format(
                len(dimensions) - 2
            )
        )
    ...
    if len(dropouts) != (len(dimensions) - 2):
        raise ValueError(
            "There must be as many dropout values as there are layer dimensions minus 2 (here {0})".format(
                len(dimensions) - 2
            )
        )
    ...
    if any([v not in accepted_functions for v in activations]):
        raise ValueError(
            "The activation function should be one of those : 'relu','sigmoid','softmax','softplus', 'softsign','tanh','selu', 'elu','exponential','linear'"
        )

    if dimensions[-1] != dimensions[0]:
        raise ValueError(
            "The dimension of the first layer should be equal to the dimension of the latest"
        )

    if not all(isinstance(x, float) for x in alphas):
        raise ValueError("All values of alpha need to be a float")

    if not all(isinstance(x, float) for x in dropouts) or all(
        (x < 0) | (x > 1) for x in dropouts
    ):
