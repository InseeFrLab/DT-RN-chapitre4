...
def QuerySingleSeries(series):
    ...
    try:
        raw1 = urlopen(
            "https://sdw-wsrest.ecb.europa.eu/service/data/" + flowREF + "/" + key
        )
        raw2 = raw1.read().decode("utf8")
        raw3 = parse(raw2)
        raw4 = raw3["message:GenericData"]["message:DataSet"]["generic:Series"][
            "generic:Obs"
        ]
        table = pd.DataFrame(
            [
                [
                    series.upper(),
                    x["generic:ObsDimension"]["@value"],
                    x["generic:ObsValue"]["@value"],
                ]
                for x in raw4
            ],
