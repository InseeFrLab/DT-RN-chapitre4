...
def load_lists():
    ...
    return var_quali, var_quanti, var_index, var_dates, var_to_drop, var_replace_minus1, var_replace_99999
...
def remove_variables_not_in_df(df, var_quali, var_quanti, var_index, var_dates, var_to_drop, var_replace_minus1, var_replace_99999):
    """ 
    ...
    extra = [x for x in var_to_drop+var_replace_99999+var_replace_minus1+var_dates+var_index+var_quanti+var_quali if x not in columns_df]
    for liste in var_to_drop,var_replace_99999,var_replace_minus1,var_dates,var_index,var_quanti,var_quali:
        for x in extra:
            if x in liste:
                liste.remove(x)
    return var_quali, var_quanti, var_index, var_dates, var_to_drop, var_replace_minus1, var_replace_99999
...
def create_quali_multimod_list(df, var_quali, threshold):
    ...
    if threshold == None:
        ...
        var_quali_multimod = [var for var in var_quali if len(pd.unique(df[var]))>threshold]
...
def create_quali_df(df, var_quali, prefix_sep = '__',dummy_na=True, drop_first= False):
    ...
    """
    df_quali = pd.get_dummies(df[var_quali],
...
def regroup_df(list_of_dataframes, how='inner', right_index=True, left_index=True):
    ...
    """
    return reduce(lambda left,right: pd.merge(left,right, how=how, left_index=left_index,right_index=right_index), list_of_dataframes)

def custom_train_test_vali_split(df, index=True, percentage=[70, 10, 20], random_seed=257):
    """
    ...
    if index == False:
        ...
        print('Les taille (nombre de lignes) des échantillons de training, de validation et de testing sont les suivantes: ', sizes)
...
def create_metadata(var_quanti, var_quali, var_quali_multimod, exceptions, total_columns):
    """
...
def normalisation_df(train, test, vali, scaler, exceptions, FillingMethod = "Mean"):
    ...
    train_scaled = pd.DataFrame(scaler.fit_transform(train[to_scale]).astype(np.float32), index = train.index, columns = train[to_scale].columns)
    test_scaled = pd.DataFrame(scaler.transform(test[to_scale]).astype(np.float32), index = test.index, columns = test[to_scale].columns)
    vali_scaled = pd.DataFrame(scaler.transform(vali[to_scale]).astype(np.float32), index = vali.index, columns = vali[to_scale].columns)
...
def check_all_nan(liste_df):
    ...
    """
    to_remove = list(set([col for df in liste_df for col in df.columns if df[col].isna().sum() == len(df)]))
    for df in liste_df:
...
def construct_AE_input_from_cleaned_df(df, var_quali, var_quanti,var_ref, threshold=None, percentage=[70,10,20], scaler=MinMaxScaler(), ManualVectorisationMethod = "Mean", FillingMethod = "Mean", random_seed = 257):
    """
    ...
    if not var_quali:
        ...
        var_quali_multimod, var_quali = create_quali_multimod_list(df, var_quali, threshold=threshold)
            ...
            for variable in tqdm(var_quali_multimod):
                for ref in var_ref:
                    if ManualVectorisationMethod == "Mean":
                        means_dict = {index:row for index, row in df_cropped.groupby(variable, dropna=False)[ref].mean().iteritems()}
                    elif ManualVectorisationMethod == "Median":
                        means_dict = {index:row for index, row in df_cropped.groupby(variable, dropna=False)[ref].median().iteritems()}
                    else:
                        raise ValueError("This method is not supported.\nImplemented methods are 'Mean' and 'Median'")
                    vectorised_df[variable+'__'+ref]=df_cropped[variable].apply(lambda x: getManualVectorisation(x, means_dict))
            print('DONE!')
    ...
    if 'df_quanti' in locals():
        if 'df_quali' in locals():
            ...
            not_to_scale = df_quali.columns.tolist() + [x for x in df_quanti.columns.tolist() if '__nan' in x]
    ...
    meta = create_metadata(var_quanti, var_quali, var_quali_multimod, not_to_scale, df_total.columns)
    meta = update_metadata(meta, ("all", df_total))
    print('DONE!')  

    #Split test train vali
    print('Splitting in training, validation and testing set...')  
    df_train, df_vali, df_test = custom_train_test_vali_split(df_total, percentage=percentage, random_seed=random_seed)
    print('DONE!')  

    #Update Meta:
    print('Updating Meta Dict...')
    meta = update_metadata(meta, ("train",df_train), ("vali",df_vali),("test",df_test))
    print('DONE!')
    #Normalization
    print('Normalizing training, validation and testing set...')  
    train_scaled, test_scaled, vali_scaled, scaler = normalisation_df(df_train, df_test, df_vali, scaler=scaler, exceptions=not_to_scale, FillingMethod = FillingMethod)
    print('DONE!')

    #Check NaNs
    print('Checking for columns full of nan...')
    train_scaled, test_scaled, vali_scaled = tuple(check_all_nan([train_scaled, test_scaled, vali_scaled]))
    print('DONE')
